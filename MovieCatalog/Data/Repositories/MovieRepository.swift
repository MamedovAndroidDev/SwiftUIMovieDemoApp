...
final class MovieRepository : MovieRepositoryProtocol {
    ...
    func fetchSimilarMovies(id: Int, page: Int) async throws -> MovieResult {
        let dto : MovieResponseDto = try await networkService.makeRequest(MovieEndpoint.fetchSimilarMovies(id: id, page: page))
        return dto.toDomain()
    }
    
    
    func fetchMovies(category: MovieCategory, page: Int) async throws -> MovieResult {
        let dto : MovieResponseDto = try await networkService.makeRequest(MovieEndpoint.movies(category: category, page: page))
        return dto.toDomain()
    }
    ...
    func searchMovies(querry: String, genreId: Int?, year: Int?) async throws -> [Movie] {
        if querry.trimmingCharacters(in: .whitespaces).isEmpty {
            let dto: MovieResponseDto = try await networkService.makeRequest(MovieEndpoint.discover(genreId: genreId, year: year, page: 1))
            return dto.results.map { $0.toDomain() }
            
        }else {
            let dto:MovieResponseDto = try await networkService.makeRequest(MovieEndpoint.search(query: querry, page: 1))
